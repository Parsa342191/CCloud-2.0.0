...
object StorageUtils {
    ...
    fun saveAllFavorites(context: Context, favorites: List<FavoriteItem>) {
        try {
            val jsonString = Json.encodeToString(favorites)
            val file = File(context.filesDir, "favorites.json")
            file.writeText(jsonString)
            Log.d(TAG, "Saved ${favorites.size} favorites (bulk)")
        } catch (e: Exception) {
            Log.e(TAG, "Error saving all favorites", e)
        }
    }
    ...
    // Bulk save - used by BackupUtils when importing a backup
    fun saveAllFavoriteGroups(context: Context, groups: List<FavoriteGroup>) {
        try {
            val jsonString = Json.encodeToString(groups)
            val file = File(context.filesDir, "favorite_groups.json")
            file.writeText(jsonString)
            Log.d(TAG, "Saved ${groups.size} favorite groups (bulk)")
        } catch (e: Exception) {
            Log.e(TAG, "Error saving all favorite groups", e)
        }
    }
    ...
    // Bulk save - used by BackupUtils when importing a backup
    fun saveAllWatchedEpisodes(context: Context, watchedEpisodes: List<WatchedEpisode>) {
        try {
            val jsonString = Json.encodeToString(watchedEpisodes)
            val file = File(context.filesDir, "watched_episodes.json")
            file.writeText(jsonString)
            Log.d(TAG, "Saved ${watchedEpisodes.size} watched episodes (bulk)")
        } catch (e: Exception) {
            Log.e(TAG, "Error saving all watched episodes", e)
        }
    }
